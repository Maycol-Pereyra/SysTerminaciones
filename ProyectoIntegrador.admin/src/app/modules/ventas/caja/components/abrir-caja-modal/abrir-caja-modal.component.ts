...
@Component({
  selector: 'app-abrir-caja-modal',
  templateUrl: './abrir-caja-modal.component.html',
})
export class AbrirCajaModalComponent extends FormBase implements OnInit, OnDestroy {
  @Input() id: number;
  ...
  abrir(): void {
    ...
    this.confirmacion(`¿Está seguro de abrir la caja?`, 'Confirmación', () => {
      const sb = this.service.abrirCaja(this.vm)
      .subscribe((res: any) => {
        if (res && res.id) {
          this.mensajeOk(`Se ha abierto la caja`);
          this.modal.close();
        } else {
